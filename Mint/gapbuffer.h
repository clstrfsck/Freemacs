...
public:
  ...
  static const mintcount_t DEFAULT_SIZE = 65536;

  // Constructors and initial set up
  explicit GapBuffer(mintcount_t size = DEFAULT_SIZE)
    : _toptop(0), _bottop(0), _topbot(size), _botbot(size), _buffer(size) { }

  mintcount_t free() const { return _topbot - _bottop; }
  mintcount_t allocated() const { return _botbot - _toptop; }
  mintcount_t size() const { return allocated() - free(); }
  ...
  const_iterator begin() const { return GapBufferIterator(this, 0); }
...
private:
  ...
  // Note that in this implementation, _toptop = 0 and _botbot = _buffer.size() always
  mintcount_t _toptop;
