...
#ifndef _EMWINDOW_CURSES_H
#define _EMWINDOW_CURSES_H

#include "emacswindow.h"

#include <map>

class EmacsWindowCurses : public EmacsWindow {
public:
    ...
    void setForeColour(int colour)      { _fore = colour; }
    int  getForeColour() const          { return _fore; }
    void setBackColour(int colour)      { _back = colour; }
    int  getBackColour() const          { return _back; }
    void setCtrlForeColour(int colour)  { _ctrl_fore = colour; }
    int  getCtrlForeColour() const      { return _ctrl_fore; }
...
private:
    // This is a void * so that we don't need to #include ncurses here
    void *_win;
    ...
    // Helper function to set up curses colours
